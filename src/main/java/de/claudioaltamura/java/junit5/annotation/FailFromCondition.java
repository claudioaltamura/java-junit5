...
public class FailFromCondition implements ExecutionCondition {
  private static final ConditionEvaluationResult ENABLED =
      ConditionEvaluationResult.enabled("@FailFrom not present or date not yet reached");
  ...
  public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
    ...
    if (annotation.isPresent()) {
      ...
      if (LocalDate.parse(failFrom.date()).isBefore(LocalDate.now())) {
        throw new AssertionFailedError(
            "Date passed: " + failFrom.date() + ". " + failFrom.reason());
      }
    }
